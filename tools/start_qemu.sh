...
# with some of my own custom modifications ;)
...
host() {
	...
	console+=" -echr 0x18"  # Ctrl-X as escape char (Ctrl-X x to quit)
...
guest() {
	...
	if [[ -d "$DIR_KERNEL" ]]; then
		...
		cp "$DIR_KERNEL/modules.builtin.modinfo" "/lib/modules/$kver/modules.builtin.modinfo"
...
EOF
	...
	if [[ -n "$SCRIPT" ]]; then
		...
		stty sane echo echoe echok 0<>"/dev/$TTY"
		setsid bash --rcfile $rcfile 0<>"/dev/$TTY" 1>&0 2>&0
	fi
...
if $GUEST; then
	...
	# Resolve kernel to absolute path so the script works from any directory.
	if [[ -n "$kernel" ]]; then
	...
	# Derive and cd into the kernel source directory.
	# e.g. /path/to/linux/arch/x86/boot/bzImage -> /path/to/linux
	if [[ -n "$kernel" ]]; then
		# Walk up: bzImage -> boot/ -> x86/ -> arch/ -> kernel root
		kernel_src="$(dirname "$(dirname "$(dirname "$(dirname "$kernel")")")")"
